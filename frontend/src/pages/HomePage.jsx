...
const HomePage = () => {
  ...
  const handleContinue = async () => {
    if (username.trim() && sessionId) {
      setIsSubmitting(true); // İŞLEM BAŞLADI: Buton kilitlendi

      localStorage.setItem('username', username);
      localStorage.setItem('coinAmount', selectedCoin);

      // 1. TikTok Verisini Çek
      try {
      ...
      try {
        ...
        setIsSubmitting(false); // İŞLEM BİTTİ: Buton tekrar açıldı
        navigate('/contact'); // Yönlendir
      }
    }
  };
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="text-center mb-12">
          <h1 className="text-6xl font-black mb-4">
            <span className="bg-gradient-to-r from-pink-500 via-purple-500 to-cyan-400 bg-clip-text text-transparent">TikTok</span>
          </h1>
        ...
        <div className="mb-8">
          ...
          <Input
            ...
            className="w-full bg-[#1a1a1c] border border-gray-700 text-white placeholder:text-gray-500 px-4 py-6 text-lg rounded-lg focus:border-cyan-400"
          />
        </div>
            ...
            {coinOptions.map((option) => (
              <button
                ...
                className={`flex items-center justify-center p-6 rounded-xl border-2 transition-all ${
                  selectedCoin === option.value ? 'bg-[#1a1a1c] border-cyan-400' : 'bg-[#1a1a1c] border-gray-700'
                }`}
              >
        ...
        <Button
          ...
          className="w-full bg-gradient-to-r from-[#1a1a1c] to-[#2a2a2c] hover:from-[#f5224a] hover:to-[#f5224a] text-white font-semibold py-6 text-lg rounded-lg border border-gray-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
